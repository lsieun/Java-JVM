 ...
 */
public final class CodeExceptionGen implements InstructionTargeter {
     ...
     */
    public ExceptionTable getCodeException(final ConstantPoolGen cp) {
        return new ExceptionTable(start_pc.getPosition(), end_pc.getPosition()
                + end_pc.getInstruction().getLength(), handler_pc.getPosition(),
                (catch_type == null) ? 0 : cp.addClass(catch_type.getClassName()));
    }
